...
INSTRUCTIONS: Final[dict[TaskClass, str]] = {
    "classify": (
        ...
        "the card-stage gate catalogue and stay inside each parameter's stated range; a "
        "parameter typed `instrument` names one id of the hypothesis's universe. "
        "Include every gate the catalogue marks required. The hypothesis may already "
        "require gates of its own, listed under `required_constraints`: those are the "
        "operator's and stand whatever you answer, so do not repeat one to change its "
        "parameters — an entry naming a gate already required is dropped. Name a gate "
        "there only if you would choose it anyway and would run it as they set it.\n\n"
    ...
    "propose": (
        ...
        "worth a card. `recent_cards` is what has already been tried, newest last, and "
        "`last_diff` is the change the card before this one made: a diff that reproduces "
        "bytes already carded is refused with the card it repeats, and reverting a kept "
        "change without a reason wastes the run. When a `crash_tail` is given, the last "
        "card raised and that is the end of its traceback, so the crash is the change to "
        "make. `failing_certification_gates` names, by id, the gates the newest failed "
        "certificate did not pass. When `rewound_for` is "
        ...
        "When a `repair` fact is given, the last card crashed and this turn is for fixing "
        "it, not for a new experiment. The idea in that card was never judged — it raised "
        "before it could be — so keep the idea and repair the fault. `repair.diff` is the "
        "change that crashed, written against the exact file you have now, `repair.desc` is "
        "what it was trying to do, and `crash_tail` is the end of the traceback it "
        "produced. Return that change again with the error corrected and nothing else "
        "altered; returning it unchanged reproduces bytes already carded and is refused. "
        "`repair.attempt` of `repair.of` says which repair this is, and when they run out "
        "the idea is dropped and the next turn asks for a fresh one — so read the traceback "
        "and fix the named fault rather than guessing around it.\n\n"
        "A card that ran is judged a second time by what it held. Its **signature** is, for "
        "each session of the research window, the instruments and sides it held and whether "
        "each was still open at the session's end. A proposal that does not beat the best, "
        "whose signature matches a strategy already judged on nearly every shared session, "
        "and whose metric comes out within this objective's noise floor of that strategy's, "
        "is `redundant`: it is recorded and it counts as a trial, but it is not an "
        "experiment, it can never be a keep, and the turn is spent for nothing. A threshold "
        "nudged, a size changed, a helper renamed or a condition spelt the other way round "
        "usually leaves the book exactly as it was and the number with it. The `redundant` "
        "fact lists the newest books already measured under this hypothesis: what was "
        "tried, the card whose book it matched as `like`, the share of their shared "
        "sessions they matched on as `pct`, and `repeat`. A `repeat` of `true` was "
        "refused as above; a `repeat` of `false` matched that book and moved the metric "
        "further than the noise floor, so it was an experiment and was carded as a "
        "discard — the book is measured either way, and holding it again is a turn spent "
        "on a book this hypothesis already knows. Read it as a list of books already "
        "measured, and change what the strategy "
        "holds and when — a different instrument, a different side, a different set of "
        "sessions, or carrying a position over a session's end where the book it repeats "
        "closed before it — rather than how the same holding is written. Both halves refuse "
        "together, so a change that holds a book already measured and moves the metric "
        "further than the noise floor is an experiment and is carded as a discard; a change "
        "that moves neither is the turn spent for nothing. The reading is by session, so "
        "moving the same holding to another hour of the same session does not make it "
        "another book.\n\n"
