...
INSTRUCTIONS: Final[dict[TaskClass, str]] = {
    ...
    "propose": (
        ...
        "and whose signature matches a strategy already judged on nearly every shared "
        "session is `redundant`: it is recorded and it counts as a trial, but it is not an "
        ...
        "usually leaves the book exactly as it was. The `redundant` fact lists the newest of "
        "them under this hypothesis: what was tried, the card it repeated as `like`, and the "
        "share of their shared sessions they matched on as `pct`. Read it as a list of books "
        "already measured, and change what the strategy holds and when — a different "
        "instrument, a different side, a different set of sessions, or carrying a position "
        "over a session's end where the book it repeats closed before it — rather than how "
        "the same holding is written. The reading is by session, so moving the same holding "
        "to another hour of the same session does not make it another book.\n\n"
