...
- **Splits are declared on the instrument, not loaded as data.** An equity that has split inside a window carries `override.info.splits: [{ex_date: <date>, ratio: <shares after per share before>}]` in `instruments.yaml`; the simulated venue applies each one at its ex-date instead of letting a run read the restated price as a return. `ex_date` is the first session that trades at the new price — the vendor's execution date, never the calendar day after it — and the split takes effect at the midnight that opens it in New York, before that session's first print, for daily and intraday bars alike. Add the entry, `kanso data instruments resolve --refresh`, then `kanso data snapshot` — the schedule is part of the definition and therefore of the snapshot. A run whose window holds a `corporate_action` split the definition does not schedule is refused (exit 2) naming the instrument, the ex-date and the ratio; that is the message to act on, never to work around. A schedule carries no cash: a dividend has an announcement date and belongs in the data.
