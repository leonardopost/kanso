...
6. `kanso data snapshot` → freezes all current datasets into `catalog/snapshots/<snapshot_id>.yaml`. `kanso research begin` pins the newest snapshot that covers the hypothesis's universe × data requirements over its research and certification windows; load and snapshot **before** beginning a run.
...
- After loading, `kanso data sync` extends existing datasets to today without touching anything a pinned snapshot references.
