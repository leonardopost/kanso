...
5. `kanso data show` → datasets, served spans, the ranges between them the source answered empty (`empty`, counted as coverage), gaps and row counts per instrument/type. Quote the **served** span, never the range that was requested: a source may return less than asked with no warning. A gap is what `kanso data backfill` asks for again — a weekend or holiday at a chunk edge is one until then, and closes when the source answers it empty. kanso keeps no calendar, so a weekday under `empty` that the venue traded is a day the source holds nothing for: tell the operator.
...
- **Splits are declared on the instrument, not loaded as data.** An equity that has split inside a window carries `override.info.splits: [{ex_date: <date>, ratio: <shares after per share before>}]` in `instruments.yaml`, with `override.info.timezone` naming where its sessions are dated (`America/New_York` for a US listing); `ex_date` is the first session that traded at the new price, and the simulated venue applies each split just after that day's midnight in that zone, between sessions for minute and daily bars alike, instead of letting a run read the restated price as a return. Without a zone the day is a UTC day, which lands a US split inside a winter post-market. Add the entry, `kanso data instruments resolve --refresh`, then `kanso data snapshot` — the schedule is part of the definition and therefore of the snapshot. A run whose window holds a `corporate_action` split the definition does not schedule is refused (exit 2) naming the instrument, the ex-date and the ratio; that is the message to act on, never to work around. A schedule carries no cash: a dividend has an announcement date and belongs in the data.
