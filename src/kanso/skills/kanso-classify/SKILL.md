...
## Rules
- Classification precedes research: the harness and the objective (absolute for a sleeve, relative to the pinned host for attached constructs) follow from the construct; a `benchmark: {hold: first_leg}` the operator declared turns a daily sleeve's absolute Sharpe into `wf_sharpe_vs_hold`, a benchmark on a hypothesis held under a day exits 3 at `hyp validate` and at `kanso classify` before any model call, and one on a construct measured relative to its host (every attached construct but `alpha`) exits 3 at `kanso classify` once the construct is chosen — never drop the key to make classification pass, tell the operator.
