...
1. `kanso cert plan <id> --json`. The `certify_plan` task class reads the hypothesis, the gate toolbox (each gate's `meaningful_when`, params and ranges — the toolbox has no defaults and no thresholds of its own), data availability, the construct and the trial count — never research results — and returns the cert/paper/live gates, each with parameters chosen inside its ranges and a rationale, plus the gates it considered and excluded, with reasons. kanso validates the plan against the invariants (every required gate this version can run, ≥1 cert gate, ≥1 paper gate, ≥1 live gate, params inside ranges), retries once with the errors, and pins it at `certificates/<id>/plan.yaml`. A cert-stage gate this version has no implementation for is neither offered to the planner nor accepted from it, so it is absent from the toolbox and from the plan until its implementation ships. Without a configured model the step exits 2: there is no default plan by design.
