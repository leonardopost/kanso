...
- Preconditions (exit 2): status `classified|candidate|researching|certified` with no active run, a runnable construct (a construct this version cannot run exits 2 and names itself), a snapshot covering the universe and windows (`kanso data snapshot`), envelope detected (`kanso env detect`), every tier has a model (`kanso models check`).
