...
- `keep` = beat the noise floor and passed card gates; `discard` = equal/worse or a card gate failed (including `strategy_integrity`, and under a `sizing` rule the `sizing` gate a refused order records); `crash` = an exception (the traceback tail is fed to the next proposal) or over budget.
