...
2. Fill `hypothesis.yaml` from the operator's words. Ask at most one question if a required field cannot be inferred; otherwise choose and state the assumption.
   ...
   - `benchmark: {hold: first_leg}` when the operator says the strategy has to beat holding the market it trades: the objective becomes the Sharpe over a hold of `universe[0]` (put the leg to beat first), run by the same runner. Only for a horizon of a day or more on a sleeve or an alpha — `kanso hyp validate` exits 3 otherwise. It is scope: adding it, or reordering the universe under it, clears `best`.
