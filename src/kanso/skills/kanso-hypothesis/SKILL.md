...
2. Fill `hypothesis.yaml` from the operator's words. Ask at most one question if a required field cannot be inferred; otherwise choose and state the assumption.
   ...
   - `costs`: keep the venue's defaults when the thesis requires `quote` data. Without `quote` there are no quotes to take a spread from, so set `costs: {spread: fixed_bps, fixed_bps: <width>}` (or rely on `venues.<MIC>.costs.fixed_bps` in `portfolio.yaml`); otherwise step 3 exits 3 naming `costs.fixed_bps`. State the width you chose. When the thesis trades limit orders that rest on the book, two more keys belong there if the operator names them: `maker_bps`, what a fill that rested pays instead of commission, slippage and half the spread (negative is a rebate; left out, it pays what any fill pays), and `limit_fill: through`, which fills a resting limit only once a bar's low or high or a print goes beyond its price rather than when the market merely reaches it (the default, `touch`).
   ...
   - `benchmark: {hold: first_leg}` when the operator says the strategy has to beat holding the market it trades: the objective becomes the Sharpe over a hold of `universe[0]` (put the leg to beat first), run by the same runner. Only for a horizon of a day or more — step 3 exits 3 on a shorter one, draft or not — and only on a sleeve or an alpha, which `kanso classify` decides. It is scope: adding it, or reordering the universe under it, clears `best`. On a file already classified, add it in the same edit as `objective.id: wf_sharpe_vs_hold` (params unchanged), or clear `construct`, `objective` and `constraints` and classify again; the key alone exits 3 with that remedy.
