...
decides between them; a sub-daily sleeve and every attached construct are unaffected by the
key, and `kanso hyp validate` refuses one that declares it. The hold is a run, not a price
