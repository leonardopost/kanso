...
| `kanso align check ID` | run the alignment check now: the deterministic syntax-tree checks first, the model only when they pass and the lane holds bytes nothing has answered for. **The run's base is never judged**: no model proposed the bytes a run was handed, and no rewind can go behind them. Nor is the file the last check left the lane on — the one it passed or the one it rewound to — which the loop returns to only when nothing has kept since. A lane on either is reported aligned with no model call and no escalation, the check is recorded so the next one differences from there, and no card's mark changes; the syntax tree still reads it, since what that finds is a fact rather than an opinion. Drift is not an error and does not exit like one — a check that finds the run has wandered has already rewound the lane to the last aligned keep, re-pointed `best`, marked the cards since the last check and written an escalation, and reports that with exit 0 |
