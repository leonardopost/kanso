...
hypotheses, runs, cards and events, stamped with the newest migration 0.7.0 shipped, and
emitted by `sqlite3.Connection.iterdump()` so that it loads under every SQLite a supported
Python links. A fresh database proves a migration's SQL parses; only a populated earlier
one proves it runs over rows written under the old shape, which is what a release has to
know.
"""
...
def test_the_fixture_is_a_populated_workspace_the_previous_release_wrote(
    ...
    text = PREVIOUS_RELEASE.read_text(encoding="utf-8")
    assert "kanso 0.7.0" in text.splitlines()[0]
    # A `.dump` from SQLite 3.50 writes a value holding a control character as `unistr(...)`,
    # a function the 3.45 CI links lacks; measured, the `iterdump()` form loads under 3.40.1.
    sql = [line for line in text.splitlines() if not line.startswith("--")]
    assert not any("unistr(" in line for line in sql)
...
def test_every_later_migration_applies_over_the_previous_release_and_keeps_its_rows(
    ...
    assert PREVIOUS_RELEASE_VERSION <= SCHEMA_VERSION, "the fixture is ahead of the package"
    before = counts(previous_release_db)
    # Empty on a branch that ships no migration after 0.7.0; the first later file makes
    # this test apply it over the rows, which is what it is here for.
