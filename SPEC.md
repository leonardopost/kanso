...
| `data/adapters/<vendor>/` (0.1.0: `massive`) | per vendor: client (`HttpClient`, header auth, quotas), reference resolution, bulk and incremental loaders, vendor-introduced custom types, static calendars, entitlement and history-floor probing |
...
Costs are applied once, by the runner, in the single extraction that feeds cards, certification gates, `expectation` and the realised paper and live objectives, rather than inside the simulated venue. One application means one number, identical across backtest, replay and broker paper, so `cost_stress` recomputes from recorded fills without re-running a backtest, two strategies with different cost models can share a stage node, and `fill_quality_drift` measures exactly the gap between the modelled cost and the broker's realised fill. The realised half of that gap is the one number a `CardRun` does not carry — every recorded `Fill` carries the cost the extraction charged, not the price the order gave up — so it is measured by the stage node from the broker's own fills and recorded on the stage result; until a node runs a broker client (§12) the gate skips for want of a measurement rather than judging one it does not have.
...
| `kanso portfolio show` · `kanso portfolio clients` · `kanso portfolio deploy --stage paper\|live` | show; list every execution client with its `capital` and `clock` declarations, the stages it may be configured on and where each credential name resolves from (never a value), and what `deploy` would refuse each stage for; validate limits, render the node configuration, (re)start the stage node |
...
`deploy --stage S` validates `limits`, renders the node configuration (strategies, capital, `RiskEngineConfig.max_notional_per_order` from `per_strategy_max_pct`), and (re)starts the stage node with the stage's `exec`, `data`, `speed`. Capital: a new version of a deployed strategy inherits its predecessor's; otherwise `min(per_strategy_max_pct × stage capital, unallocated)`, and `0` → escalation `deploy_blocked`. A stage keeps one **session clock** (its persisted replay position); restarts resume from it and a joining version records `joined_at`. `deploy` refuses (exit 2) a stage whose `kill_switch` is on, an `exec` id nothing in the workspace provides, a version whose `pins.nautilus_version` differs from the installed engine, or a stage with no catalog data at or after `forward.start`. **It also refuses (exit 2) a `clock: wall` execution client outright while the stage node remains a bounded run** — the node releases what the catalog holds that the stage has not replayed, into the simulated venue, flattens and returns, which is what a `clock: replay` client declares it is executed by; a wall-clock client fills against current prices and needs a node that outlives the command, and running one through this node would fill every order in simulation while the stage record, and the paper and live gates reading it, called the money the broker's. That refusal is made *after* the per-version approval check, so a `capital: real` client with no approval still exits 4 for the approval it is missing. It is lifted by the long-lived node (§12), not by an adapter. A node flattens — cancels open orders and closes positions — before every stop, so a stage always restarts flat; simulated execution keeps no position across a restart, so the alternative is losing positions silently, and each redeploy therefore realises its P&L into the record the paper and live gates read (stated in `paper_forward`'s `meaningful_when`, since it shortens the useful window).
...
| execution safety | exit 4 for a `capital: real` execution client off the live stage or without a recorded approval; exit 2 for a `clock: wall` client paired with replay data, with `speed ≠ 1`, or at all while the stage node is a bounded run (§8.6); no credential appears in any log, `--json` output, session file, manifest, repr or `doctor --report`; the kill switch cancels and flattens through the broker path once a node outlives the command that starts it (§12) |
...
| M3 | certification planner; runner and certificates; `cert`; failure feedback; the required `publication_lag` gate joins the toolbox; the escalation inbox a certification failure needs — the append-only file, the read state and `inbox` | plan and certification tests |
| M4 | composition and expectations; `ReplayDataClient`; sessions; sandbox nodes; `replay`; portfolio deployment; monitor; promote/demote; the escalation webhook and the deployment escalation kinds | deployment and e2e tests |
...
| M6 | **Deferred out of 0.1.0 by the operator.** The EDGAR and FINRA adapters were scoped and their sources measured, but SEC refuses this build host's command-line egress, so the adapter's own transport could never be exercised against its source here. Rather than ship a vendor whose request path has never run, both move to the additional-adapters seam (§12) and 0.1.0 ships without them. | — |
| M7 | **Alpaca execution and live market data adapter.** `alpaca_paper` and `alpaca` execution ids; `LiveExecutionClient` with the five report generators; the unsupported-order deny table; the deterministic trade id; the tradability overlay; the declared market-data feed; `capital` and `clock` declarations with the D13 guard; kanso's own simulated venue in place of the engine's convenience client; `fill_quality_drift` | a restart produces zero duplicate fills; every `deploy` refusal; the credential-leak scan; `promote --live --as NAME` remains the only path to a `capital: real` client; the two feeds distinguished; a paper key refused against the live host and the reverse; an Alpaca and a Massive daily bar for one session agree on `ts_event`. The long-lived node a `clock: wall` client needs is **§12**, so sandbox-versus-broker order-intent parity on a live stage is deferred with it; the stage-node half of that comparison — kanso's venue against the research path over the same session — is asserted instead |
...
Order: M0 → M1 → M2 → M3 → M4 → M5 → M7 → M8 (M6 deferred). Adapters follow M4 so the core is provably complete and offline-green before any vendor lands, the e2e and demo stay synthetic, and the only milestone that can move money has composition, replay, sessions and deployment behind it.
...
| additional adapters | any vendor or broker through `Adapter` + `Loader` + `InstrumentProvider` (data) or a `LiveExecutionClient` factory registered against an exec id (execution). **SEC EDGAR and FINRA are the named first candidates**: both were scoped and their shapes measured — EDGAR's acceptance instant lives in `submissions` and joins company facts on the accession, its `frames` rows carry no publication instant and are therefore unstampable, and FINRA's daily short-sale file ends in a bare row count whose disagreement with the rows above it fails the load — and neither is in 0.1.0, in the package or in `kanso_ext/`; NautilusTrader's own adapters are reachable through the same exec and data id mechanism. A third-party distribution declares its own dependencies; the kanso distribution declares none (C3) |
| the long-lived stage node | A stage node in 0.1.0 is a bounded run: `deploy` releases what the catalog holds that the stage has not replayed, into kanso's own simulated venue, flattens and returns. A `clock: wall` execution client needs a node that outlives the command that started it, so `deploy` refuses one (exit 2, §8.6) rather than filling its orders in simulation and recording them as the broker's. The seam is that node: the broker's execution and live data clients registered in place of the replay client and the simulated venue, with `reconciliation` on and non-zero inflight, open-order and position check intervals (the shipped adapter reads no order stream); venue routing per MIC; the kill switch read from the state store on the node's own timer, which is what makes §8.6's kill switch cancel and flatten through the broker path; and a realised window built from the broker's own fills, which is also the measurement channel `fill_quality_drift` waits on. Nothing about the adapter interface changes |
...
| `fill_quality_drift` | gate / live | — | rolling realised slippage minus `costs.slippage_bps` ≤ `max_excess_bps` over ≥ `min_fills`; `skipped` on a simulated execution client, on one whose declaration cannot be resolved (which a broken adapter degrades to, and which may therefore be the simulated one), and where the realised figure is absent or not finite — a non-finite measurement must never demote a live version | `max_excess_bps` [0, 50], `min_fills` [10, 10000] |
